...
describe('premium stock gateway enforcement', () => {
  it('requires credentials for premium endpoints regardless of origin', async () => {
    ...
    const browserNoKey = await handler(new Request('https://zmonitor.app/api/market/v1/analyze-stock?symbol=AAPL', {
      headers: { Origin: 'https://zmonitor.app' },
    }));
    assert.equal(browserNoKey.status, 401);

    // Trusted browser origin with a valid key — also allowed
    const browserWithKey = await handler(new Request('https://zmonitor.app/api/market/v1/analyze-stock?symbol=AAPL', {
      headers: {
        Origin: 'https://zmonitor.app',
        'X-Z-Monitor-Key': 'real-key-123',
      },
    }));
    ...
    const publicAllowed = await handler(new Request('https://zmonitor.app/api/market/v1/list-market-quotes?symbols=AAPL', {
      headers: { Origin: 'https://zmonitor.app' },
    }));
...
describe('premium stock gateway bearer token auth', () => {
  ...
  it('accepts valid Pro bearer token on premium endpoint → 200', async () => {
    ...
    const res = await handler(new Request('https://zmonitor.app/api/market/v1/analyze-stock?symbol=AAPL', {
      headers: {
        Origin: 'https://zmonitor.app',
  ...
  it('public routes are unaffected by absence of auth header', async () => {
    const res = await handler(new Request('https://zmonitor.app/api/market/v1/list-market-quotes?symbols=AAPL', {
      headers: { Origin: 'https://zmonitor.app' },
    }));
    assert.equal(res.status, 200);
  });
