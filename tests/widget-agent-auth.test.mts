...
describe('widget-agent unified tester key auth', () => {
  ...
  it('accepts X-Z-Monitor-Key and upgrades relay request to pro', async () => {
    const res = await handler(new Request('https://www.zmonitor.app/api/widget-agent', {
      method: 'POST',
      headers: {
        Origin: 'https://www.zmonitor.app',
        'Content-Type': 'application/json',
        'X-Z-Monitor-Key': 'browser-test-key',
      },
    ...
    assert.equal(call.arguments[0], 'https://proxy.zmonitor.app/widget-agent');
    ...
    assert.equal(headers.get('X-Z-Monitor-Key'), null);
  ...
  it('falls back to legacy tester keys when X-Z-Monitor-Key is invalid', async () => {
    const res = await handler(new Request('https://www.zmonitor.app/api/widget-agent', {
      method: 'POST',
      headers: {
        Origin: 'https://www.zmonitor.app',
        'Content-Type': 'application/json',
        'X-Z-Monitor-Key': 'wrong-key',
  ...
  it('rejects invalid X-Z-Monitor-Key before relay fetch', async () => {
    const res = await handler(new Request('https://www.zmonitor.app/api/widget-agent', {
      method: 'POST',
      headers: {
        Origin: 'https://www.zmonitor.app',
