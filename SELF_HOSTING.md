# 🌍 Self-Hosting Z-Monitor

Run the full Z-Monitor stack locally with Docker/Podman.
...
|-----------|---------|------|
| `zmonitor` | nginx + Node.js API (supervisord) | 3000 → 8080 |
| `zmonitor-redis` | Data store | 6379 (internal) |
| `zmonitor-redis-rest` | Upstash-compatible REST proxy | 8079 |
| `zmonitor-ais-relay` | Live vessel tracking WebSocket | 3004 (internal) |
...
# Full Docker image
docker build -t zmonitor:latest -f Dockerfile .
...
| 🔴 nginx won't start | Check `podman logs zmonitor` — likely missing `gettext` package |
...
| 🚢 No vessel data | Set `AISSTREAM_API_KEY` in both `zmonitor` and `ais-relay` services |
