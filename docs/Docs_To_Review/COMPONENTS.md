# Component Documentation — Z-Monitor
...
Z-Monitor's UI is built entirely with **vanilla TypeScript** — no React, Vue, or
...
`localStorage['zmonitor-panel-spans']` as a JSON map of `{ [id]: height }`.
...
Z-Monitor ships two independent map renderers and an adapter that picks the
...
Recent selections stored in `localStorage['zmonitor_recent_searches']`
(most recent 10).
