# Documentation Roadmap — Z-Monitor
...
- [ ] Nginx configuration (reference `deploy/nginx-zmonitor.conf`)
- [ ] SystemD service setup (reference `deploy/zmonitor-api.service`)
...
- [ ] Add entries for UI customizations on `feat/ui-customizations-zmonitor` branch
