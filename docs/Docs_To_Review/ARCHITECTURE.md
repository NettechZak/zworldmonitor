...
| **IndexedDB** | `zmonitor_db` | Client-side storage for playback snapshots and temporal baseline data. |
...
Z-Monitor ships as three product variants from a single codebase. Each variant surfaces a different subset of panels, map layers, and data sources.

| Variant | Domain | Focus |
|---|---|---|
| `full` | zmonitor.app | Geopolitics, military, OSINT, conflicts, markets |
| `tech` | tech.zmonitor.app | AI/ML, startups, cybersecurity, developer tools |
| `finance` | finance.zmonitor.app | Markets, trading, central banks, macro indicators |
...
```
localStorage('zmonitor-variant')  →  import.meta.env.VITE_VARIANT  →  default 'full'
```
...
export const SITE_VARIANT: string = (() => {
  if (typeof window !== 'undefined') {
    const stored = localStorage.getItem('zmonitor-variant');
...
graph TD
    ...
    subgraph LayerConfig["Layer Configuration"]
        ...
        UserPref["localStorage overrides<br/>(zmonitor-layers)"]
...
2. **User localStorage** — Stored under the key `zmonitor-layers`. Users toggle layers in the map controls UI, and their preferences persist across sessions.
...
Z-Monitor employs a five-tier caching strategy to minimise API costs, reduce latency, and enable offline operation.
...
graph TD
    ...
    subgraph Tier5["Tier 5: Persistent Cache"]
        ...
        LSFallback["localStorage fallback<br/>prefix: zmonitor-persistent-cache:"]
        PC --> TauriInvoke
        PC --> LSFallback
    end
...
The `zmonitor_db` IndexedDB database contains two object stores:
...
**2. Feature Toggles** — 14 `RuntimeFeatureId` values stored in localStorage under the key `zmonitor-runtime-feature-toggles`:
...
Z-Monitor runs machine-learning inference directly in the browser using ONNX Runtime Web via Transformers.js, with API-based fallbacks for constrained devices.
...
Z-Monitor uses a circuit-breaker pattern to manage transient failures across its many data sources, preventing cascade failures and providing graceful degradation.
