# Z-Monitor — API Reference

> Comprehensive reference for all Vercel Edge Function endpoints powering the Z-Monitor intelligence dashboard.

**Base URL**: All endpoints are relative to `/api/` (e.g., `https://zmonitor.app/api/earthquakes`).
...
Z-Monitor exposes **60+ serverless endpoints** deployed as **Vercel Edge Functions** (unless noted otherwise). Every endpoint:
...
| # | Pattern | Matches |
|---|---------|---------|
| 1 | `zmonitor\.app$` | `https://zmonitor.app` |
| 2 | `\.zmonitor\.app$` | `https://*.zmonitor.app` |
...
Z-Monitor uses a **multi-tier caching strategy** to minimize upstream API calls, reduce latency, and stay within third-party rate limits.
