...
> **Z-Monitor** — Config-driven panel architecture powering three site variants.
>
...
4. [Full Variant Panels](#4-full-variant-panels-zmonitorio)
5. [Tech Variant Panels](#5-tech-variant-panels-techzmonitorio)
6. [Finance Variant Panels](#6-finance-variant-panels-financezmonitorio)
...
Z-Monitor uses a **config-driven panel system** where every dashboard tile — from live news feeds to AI insights to market data — is declared as a `PanelConfig` entry inside a variant-specific configuration object. The system is designed around three principles:
...
|---------|--------|-------|-------------|
| `full` | zmonitor.io | Geopolitical intelligence, OSINT, defense | 37 |
| `tech` | tech.zmonitor.io | Technology, AI/ML, startups, VC | 34 |
| `finance` | finance.zmonitor.io | Markets, trading, macro, commodities | 29 |
...
export const STORAGE_KEYS = {
  panels:        'zmonitor-panels',          // Panel visibility toggles
  monitors:      'zmonitor-monitors',         // Monitor keyword configs
  mapLayers:     'zmonitor-layers',           // Map layer toggles
  disabledFeeds: 'zmonitor-disabled-feeds',   // Per-source feed disabling
} as const;
```
...
|-----|---------|------------|
| `zmonitor-panel-spans` | Panel height/span sizes (1–4) | `Panel.ts` |
| `panel-order` | Drag-and-drop panel ordering | `App.ts` |
| `zmonitor-variant` | Last-active variant (triggers reset on change) | `App.ts` |
| `zmonitor-panel-order-v1.9` | Migration flag for v1.9 panel layout | `App.ts` |
...
Span values are persisted per-panel in the `zmonitor-panel-spans` localStorage key as a JSON object `{ [panelId]: spanNumber }`.
...
## 4. Full Variant Panels (zmonitor.io)
...
## 5. Tech Variant Panels (tech.zmonitor.io)
...
## 6. Finance Variant Panels (finance.zmonitor.io)
...
|---------|-----------------|--------|----------------|:--------------:|
| Panel visibility | `zmonitor-panels` | `Record<string, PanelConfig>` JSON | `DEFAULT_PANELS` | ✅ |
| Panel ordering | `panel-order` | `string[]` JSON | Config declaration order | ✅ |
| Panel sizes/spans | `zmonitor-panel-spans` | `Record<string, number>` JSON | All span-1 | ✅ |
| Map layer toggles | `zmonitor-layers` | `MapLayers` JSON | `DEFAULT_MAP_LAYERS` | ✅ |
| Monitor keywords | `zmonitor-monitors` | `Monitor[]` JSON | `[]` | ✅ |
| Disabled sources | `zmonitor-disabled-feeds` | `string[]` JSON | `[]` | ✅ |
| Active variant | `zmonitor-variant` | Plain string | `SITE_VARIANT` | ✅ |
| Banner dismissal | `banner-dismissed` (sessionStorage) | Timestamp string | — | Session only |

### 9.2 Variant Change Reset

When the stored variant (`zmonitor-variant`) differs from the current `SITE_VARIANT`, the App constructor performs a full reset:

```typescript
if (storedVariant !== currentVariant) {
  localStorage.setItem('zmonitor-variant', currentVariant);
...
This ensures users switching between variant domains (e.g. from zmonitor.io to tech.zmonitor.io) get a clean default experience for the new variant.
...
Step-by-step guide to adding a new panel to Z-Monitor.
...
sequenceDiagram
    ...
    App->>localStorage: saveToStorage('zmonitor-panels', panelSettings)
    ...
    App->>localStorage: loadFromStorage('zmonitor-panels')
