...
Z-Monitor is an AI-powered real-time global intelligence dashboard built with **vanilla TypeScript** — no framework, no reactive stores. All state is managed manually through class properties, `localStorage`, `IndexedDB`, and URL query parameters.
...
Stored in `localStorage` under `zmonitor-panels` as `Record<string, PanelConfig>`. Includes per-panel `enabled` state, `name`, and `priority`. Controlled by the variant config and user overrides.
...
| Key | Type | Purpose |
|-----|------|---------|
| `zmonitor-panels` | `Record<string, PanelConfig>` | Per-panel enabled/name/priority |
| `zmonitor-monitors` | `Monitor[]` | Color/label configs for monitors |
| `zmonitor-layers` | `MapLayers` | Enabled/disabled map layer toggles |
| `zmonitor-disabled-feeds` | `string[]` | User-disabled news feed sources |
...
sequenceDiagram
    ...
    setTheme->>localStorage: setItem('zmonitor-theme', 'light')
...
export async function initDB(): Promise<IDBDatabase> {
  // Opens or creates zmonitor_db v1
...
- **Toggles key:** `zmonitor-runtime-feature-toggles` in `localStorage`
...
Complete reference of every `localStorage` key used by Z-Monitor:

| Key | Purpose | Format | Source |
|-----|---------|--------|--------|
| `zmonitor-variant` | Active variant override | `'full' \| 'tech' \| 'finance'` | `src/config/variant.ts`, `App.ts` |
| `zmonitor-theme` | Theme preference | `'dark' \| 'light'` | `src/utils/theme-manager.ts` |
| `panel-order` | Panel arrangement order | `string[]` (JSON) | `App.ts` |
| `zmonitor-panel-spans` | Panel grid sizes | `Record<string, number>` (JSON) | `src/components/Panel.ts` |
| `zmonitor-panels` | Panel enabled/config state | `Record<string, PanelConfig>` (JSON) | `STORAGE_KEYS.panels` |
| `zmonitor-monitors` | Monitor color/label configs | `Monitor[]` (JSON) | `STORAGE_KEYS.monitors` |
| `zmonitor-layers` | Map layer toggles | `MapLayers` (JSON) | `STORAGE_KEYS.mapLayers` |
