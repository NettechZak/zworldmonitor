...
| **Client — pinned store** | localStorage (permanent) | None (user-managed) | `zm-pinned-webcams` |
...
Pinned webcam data is stored in `localStorage` under the key `zm-pinned-webcams`. Each entry contains:
