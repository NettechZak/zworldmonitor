...
| Pattern | Matches |
| --- | --- |
| `(*.)?zmonitor.app` | Production + subdomains (`tech.`, `finance.`, etc.) |
| `zmonitor-*-elie-*.vercel.app` | Vercel preview deploys |
...
- `Content-Type`
- `Authorization`
- `X-Z-Monitor-Key` (API key for desktop/third-party access). See [API Key Gating](/api-key-deployment) for key management details.
