...
**Purpose:** Reference document for extending Z-Monitor's finance hub with TradingView Screener data.
...
The TypeScript library is the one relevant for Z-Monitor. It has full parity with the Python version (41/41 operations pass) plus a self-hosted REST server option.
...
Z-Monitor's data flow: `Railway seeds Redis → Vercel reads Redis → Frontend RPC`
...
```typescript
// server/zmonitor/market/v1/list-tv-stock-screener.ts
...
### Z-Monitor-Specific Recommendations
...
| **No auth initially** | Delayed data is fine for Z-Monitor's use case |
...
- [ ] Create `proto/zmonitor/market/v1/list_tv_stock_screener.proto`
- [ ] Create `server/zmonitor/market/v1/list-tv-stock-screener.ts`
