...
description: "All JSON API endpoints in Z-Monitor must use sebuf. This guide walks through adding a new RPC to an existing service and adding an entirely new service."
---
All JSON API endpoints in Z-Monitor **must** use sebuf. Do not create standalone `api/*.js` files — the legacy pattern is deprecated and being removed.

This guide walks through adding a new RPC to an existing service and adding an entirely new service.

> **Important:** After modifying any `.proto` file, you **must** run `make generate` before building or pushing. The generated TypeScript files in `src/generated/` are checked into the repo and must stay in sync with the proto definitions. CI does not run generation yet — this is your responsibility until we add it to the pipeline (see [#200](https://github.com/NettechZak/zmonitor/issues/200)).
...
Create `proto/zmonitor/seismology/v1/get_earthquake_details.proto`:

```protobuf
syntax = "proto3";
package zmonitor.seismology.v1;

import "buf/validate/validate.proto";
import "zmonitor/seismology/v1/earthquake.proto";
...
Edit `proto/zmonitor/seismology/v1/service.proto`:

```protobuf
import "zmonitor/seismology/v1/get_earthquake_details.proto";
...
Create `server/zmonitor/seismology/v1/get-earthquake-details.ts`:
...
Edit `server/zmonitor/seismology/v1/handler.ts`:
...
```
proto/zmonitor/sanctions/v1/
```

### 2. Define entity messages

Create `proto/zmonitor/sanctions/v1/sanctions_entry.proto`:

```protobuf
syntax = "proto3";
package zmonitor.sanctions.v1;
...
Create `proto/zmonitor/sanctions/v1/list_sanctions.proto`:
...
import "buf/validate/validate.proto";
import "zmonitor/core/v1/pagination.proto";
import "zmonitor/sanctions/v1/sanctions_entry.proto";
...
message ListSanctionsRequest {
  ...
  zmonitor.core.v1.PaginationRequest pagination = 3;
}
...
message ListSanctionsResponse {
  ...
  // Pagination metadata.
  zmonitor.core.v1.PaginationResponse pagination = 2;
}
```

### 4. Define the service

Create `proto/zmonitor/sanctions/v1/service.proto`:
...
import "sebuf/http/annotations.proto";
import "zmonitor/sanctions/v1/list_sanctions.proto";
...
```
server/zmonitor/sanctions/v1/
├── handler.ts               # thin re-export
└── list-sanctions.ts        # RPC implementation
```

`server/zmonitor/sanctions/v1/list-sanctions.ts`:
```typescript
import type {
...
`server/zmonitor/sanctions/v1/handler.ts`:
```typescript
...
| Type | Import | Use for |
|------|--------|---------|
| `GeoCoordinates` | `zmonitor/core/v1/geo.proto` | Any lat/lon location (has built-in -90/90 and -180/180 bounds) |
| `BoundingBox` | `zmonitor/core/v1/geo.proto` | Spatial filtering |
| `TimeRange` | `zmonitor/core/v1/time.proto` | Time-based filtering (has `INT64_ENCODING_NUMBER`) |
| `PaginationRequest` | `zmonitor/core/v1/pagination.proto` | Request pagination (has page_size 1-100 constraint) |
| `PaginationResponse` | `zmonitor/core/v1/pagination.proto` | Response pagination metadata |
