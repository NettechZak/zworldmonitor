...
# Z-Monitor PRO MCP Server

## Problem Frame

Z-Monitor accumulates, curates, and caches real-time intelligence across 25+ domains. PRO users currently access this data only through the web UI or desktop app. They cannot query it from Claude Desktop, Cursor, Windsurf, or any MCP-compatible AI agent. This creates a hard wall between Z-Monitor's data layer and AI workflows. The MCP server removes that wall: PRO API key holders point their MCP client at `https://api.zmonitor.app/mcp` and Z-Monitor data becomes natively queryable from any AI agent.
...
- A PRO user can add Z-Monitor as an MCP server in Claude Desktop using only their API key and `https://api.zmonitor.app/mcp` as the URL — no install, no CLI.
