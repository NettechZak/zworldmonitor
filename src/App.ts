...
export class App {
  ...
  private async primeVisiblePanelData(forceAll = false): Promise<void> {
    ...
    const _wmAccess = getSecretState('ZMONITOR_API_KEY').present || getAuthState().user?.role === 'pro';
    if (_wmAccess) {
  ...
  constructor(containerId: string) {
    ...
    const storedVariant = localStorage.getItem('zmonitor-variant');
    ...
    if (storedVariant !== currentVariant) {
      ...
      const PANEL_KEY_RENAMES_MIGRATION_KEY = 'zmonitor-panel-key-renames-v2.6';
      ...
      const UNIFIED_MIGRATION_KEY = 'zmonitor-unified-panels-v1';
      ...
      const HAPPY_PANEL_FIX_KEY = 'zmonitor-happy-panel-fix-v1';
      ...
      const PANEL_ORDER_MIGRATION_KEY = 'zmonitor-panel-order-v1.9';
      ...
      if (currentVariant === 'tech') {
        const TECH_INSIGHTS_MIGRATION_KEY = 'zmonitor-tech-insights-top-v1';
    ...
    const PANEL_PRUNE_KEY = 'zmonitor-panel-prune-v1';
    ...
    const LAYOUT_RESET_MIGRATION_KEY = 'zmonitor-layout-reset-v2.5';
    ...
    if (currentVariant === 'full') {
      const baseKey = 'zmonitor-sources-reduction-v3';
      ...
      const localeKey = `zmonitor-locale-boost-${userLang}`;
  ...
  private setupRefreshIntervals(): void {
    ...
    if (SITE_VARIANT === 'finance') {
      this.refreshScheduler.scheduleRefresh(
        ...
        () => (getSecretState('ZMONITOR_API_KEY').present || getAuthState().user?.role === 'pro') && this.isPanelNearViewport('stock-analysis'),
      );
      this.refreshScheduler.scheduleRefresh(
        'daily-market-brief',
        () => this.dataLoader.loadDailyMarketBrief(),
        REFRESH_INTERVALS.dailyMarketBrief,
        () => (getSecretState('ZMONITOR_API_KEY').present || getAuthState().user?.role === 'pro') && this.isPanelNearViewport('daily-market-brief'),
      );
      this.refreshScheduler.scheduleRefresh(
        'stock-backtest',
        () => this.dataLoader.loadStockBacktest(),
        REFRESH_INTERVALS.stockBacktest,
        () => (getSecretState('ZMONITOR_API_KEY').present || getAuthState().user?.role === 'pro') && this.isPanelNearViewport('stock-backtest'),
      );
      this.refreshScheduler.scheduleRefresh(
        'market-implications',
        () => this.dataLoader.loadMarketImplications(),
        REFRESH_INTERVALS.marketImplications,
        () => (getSecretState('ZMONITOR_API_KEY').present || isProUser()) && this.isPanelNearViewport('market-implications'),
      );
    }
