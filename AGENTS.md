...
│   └── zzmonitor/       # Domain handlers (mirrors proto service structure)
├── proto/                  # Protobuf definitions (sebuf framework)
│   ├── buf.yaml            # Buf configuration
│   └── zzmonitor/       # Service definitions with HTTP annotations
...
- `server/zmonitor/<domain>/` has RPC handlers matching proto services
...
1. Define proto message in `proto/zmonitor/<domain>/`
2. Add RPC with `(sebuf.http.config)` annotation
3. Run `make generate`
4. Create handler in `server/zmonitor/<domain>/`
