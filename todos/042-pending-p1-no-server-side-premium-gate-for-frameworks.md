...
## Findings
- **`server/zmonitor/intelligence/v1/get-country-intel-brief.ts`** — reads `req.framework`, truncates to 2000 chars, passes to `callLlm` as `systemAppend`. No entitlement check.
- **`server/zmonitor/intelligence/v1/deduct-situation.ts`** — same pattern, no entitlement check.
- **`server/zmonitor/news/v1/summarize-article.ts`** — reads `req.systemAppend`, no entitlement check.
...
- Flagged by: security-sentinel, architecture-strategist, learnings-researcher (zmonitor-pro-panel-gating skill)
...
## Technical Details
- Files: `server/zmonitor/intelligence/v1/get-country-intel-brief.ts`, `server/zmonitor/intelligence/v1/deduct-situation.ts`, `server/zmonitor/news/v1/summarize-article.ts`
- PR: NettechZak/zmonitor#2380
- Reference: zmonitor-pro-panel-gating skill (4-layer checklist)
