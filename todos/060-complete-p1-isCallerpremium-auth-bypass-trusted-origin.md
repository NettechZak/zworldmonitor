...
## Findings
- **`server/_shared/premium-check.ts:10-12`** — `if (keyCheck.valid) return true;` triggers for all zmonitor.app sessions
- **`api/_api-key.js:49-68`** — `isTrustedBrowserOrigin()` returns `true` for `*.zmonitor.app`, `*vercel.app`, `localhost`; causes `validateApiKey` to return `{ valid: true, required: false }` with no key present
...
## Acceptance Criteria
- [ ] Free-tier users on zmonitor.app do NOT pass `isCallerPremium` check
