...
export function validateApiKey(req, options = {}) {
  ...
  const key = req.headers.get('X-Z-Monitor-Key');
  ...
  if (isDesktopOrigin(origin)) {
    ...
    const validKeys = (process.env.Z-MONITOR_VALID_KEYS || '').split(',').filter(Boolean);
    if (!validKeys.includes(key)) return { valid: false, required: true, error: 'Invalid API key' };
    return { valid: true, required: true };
  }

  // Trusted browser origin (zmonitor.app, Vercel previews, localhost dev) — no key needed
  if (isTrustedBrowserOrigin(origin)) {
    if (forceKey && !key) {
      return { valid: false, required: true, error: 'API key required' };
    }
    if (key) {
      const rawEnv = process.env.Z-MONITOR_VALID_KEYS || '';
